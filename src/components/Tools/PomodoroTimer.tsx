...
const PHASE_CONFIG: Record<Phase, { label: string; color: string; minutes: number }> = {
  ...
  'short-break': { label: 'Short Break', color: 'text-emerald-600', minutes: 5 },
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-col items-center gap-6 py-4">
          ...
          <div className="flex gap-3">
            <button
              ...
              className={`px-8 py-2.5 rounded-xl font-semibold text-white transition-colors ${running ? 'bg-slate-600 hover:bg-slate-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}
            >
              {running ? 'Pause' : 'Start'}
            </button>
      ...
      <ToolCard title="Settings" action={
            ...
            {([['Focus', 'work'], ['Short Break', 'short'], ['Long Break', 'long']] as const).map(([label, key]) => (
              <div key={key}>
                ...
                <input
                  ...
                  className="w-full px-2 py-1.5 text-sm text-center border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                />
              </div>
            ))}
          </div>
        ...
        <div className="mt-2 flex gap-2 flex-wrap">
          <button onClick={requestNotifications} className="text-xs px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:border-emerald-400 transition-colors">
