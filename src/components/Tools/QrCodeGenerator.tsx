...
export default function QrCodeGenerator() {
  ...
  const renderFields = () => {
    const cls = 'w-full text-sm px-3 py-2.5 rounded-lg border border-slate-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-50 outline-none min-h-[44px]';
    switch (preset) {
          ...
          <div className="flex flex-wrap gap-2">
            {(Object.keys(PRESET_LABELS) as Preset[]).map((p) => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-semibold rounded-lg border min-h-[36px] transition-colors ${preset === p ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50'}`}
              >
                {PRESET_LABELS[p]}
              </button>
            ))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={ecLevel} onChange={(e) => setEcLevel(e.target.value as ErrorLevel)}
                className="w-full text-sm px-3 py-2 rounded-lg border border-slate-200 focus:border-emerald-400 outline-none min-h-[44px]">
        ...
        {dataUrl ? (
          <>
            ...
            <div className="flex flex-col gap-2">
              <button onClick={downloadPng} className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 min-h-[44px]">
                Download PNG
              </button>
