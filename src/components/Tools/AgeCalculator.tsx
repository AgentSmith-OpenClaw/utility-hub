    ...
    <div className="space-y-5">
      <div className="flex gap-2 flex-wrap">
        {([['age', 'Age Calculator'], ['diff', 'Date Difference'], ['add', 'Add / Subtract Days']] as const).map(([m, label]) => (
          <button key={m} onClick={() => setMode(m)} className={`px-4 py-1.5 text-sm rounded-lg border font-medium transition-colors ${mode === m ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-700 border-slate-200 hover:border-emerald-400'}`}>
            {label}
          </button>
        ))}
      </div>
          ...
          <ToolCard title="Date of Birth">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="date" value={dob} onChange={(e) => setDob(e.target.value)} max={today} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-500/30" />
              </div>
              <div>
                <label className="text-xs text-slate-500 mb-1 block">As of date</label>
                <input type="date" value={refDate} onChange={(e) => setRefDate(e.target.value)} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-500/30" />
              </div>
            </div>
          </ToolCard>
          {ageResult && (
          ...
          <ToolCard title="Two Dates">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="date" value={date1} onChange={(e) => setDate1(e.target.value)} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-500/30" />
              </div>
              <div>
                <label className="text-xs text-slate-500 mb-1 block">End date</label>
                <input type="date" value={date2} onChange={(e) => setDate2(e.target.value)} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-500/30" />
              </div>
            </div>
          </ToolCard>
          {diffResult && (
        ...
        <>
          <ToolCard title="Add or Subtract Days">
            <div className="flex items-center gap-3 flex-wrap">
              <input type="date" value={baseDate} onChange={(e) => setBaseDate(e.target.value)} className="px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-500/30" />
              <div className="flex gap-1">
                {(['+', '-'] as const).map((s) => (
                  <button key={s} onClick={() => setAddSign(s)} className={`w-8 h-8 rounded-lg border text-sm font-bold transition-colors ${addSign === s ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-700 border-slate-200 hover:border-emerald-400'}`}>{s}</button>
                ))}
              </div>
              <input type="number" min={0} value={addDays} onChange={(e) => setAddDays(e.target.value)} className="w-20 px-2 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-500/30" />
              <span className="text-sm text-slate-500">days</span>
            </div>
          </ToolCard>
          {addResult && (
            <ToolCard title="Result" action={<CopyButton value={addResult} />}>
              <p className="text-xl font-bold text-emerald-700">{addResult}</p>
            </ToolCard>
          )}
        </>
      )}
