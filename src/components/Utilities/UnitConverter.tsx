    ...
    <div className="space-y-5">
      <ToolCard title="Category">
        <div className="flex flex-wrap gap-2">
          {categories.map((cat) => (
            <button key={cat} onClick={() => handleCategoryChange(cat)} className={`px-3 py-1.5 text-sm rounded-lg border font-medium transition-colors ${category === cat ? 'bg-amber-600 text-white border-amber-600' : 'bg-white text-slate-700 border-slate-200 hover:border-amber-400'}`}>
              {cat}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-[1fr_auto_1fr] gap-3 items-end">
            <div>
              ...
              <select value={fromIdx} onChange={(e) => setFromIdx(+e.target.value)} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-amber-500/30">
                {units.map((u, i) => <option key={u.label} value={i}>{u.label}</option>)}
              </select>
            </div>
            <button onClick={swap} className="h-9 w-9 flex items-center justify-center rounded-lg border border-slate-200 hover:border-amber-400 text-slate-500 transition-colors text-lg mb-0.5">⇄</button>
            <div>
              <label className="text-xs text-slate-500 mb-1 block">To</label>
              <select value={toIdx} onChange={(e) => setToIdx(+e.target.value)} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-amber-500/30">
                {units.map((u, i) => <option key={u.label} value={i}>{u.label}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" value={value} onChange={(e) => setValue(e.target.value)} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-amber-500/30" />
            </div>
            <div>
              <label className="text-xs text-slate-500 mb-1 block">Result</label>
              <div className="flex gap-2">
                <div className="flex-1 px-3 py-2 text-sm font-mono font-semibold text-amber-700 border border-slate-200 rounded-lg bg-slate-50">{result || '—'}</div>
