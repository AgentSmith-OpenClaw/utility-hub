...
/**
 * Parse a page range string like "1-5, 8, 10-12" into a set of 0-indexed page numbers.
 * Throws descriptively on invalid input.
 */
...
async function deletePages(file: File, pagesToDelete: Set<number>, PDFDocument: typeof import('pdf-lib').PDFDocument): Promise<Blob> {
...
export default function DeletePdfPages() {
  ...
  const handleFiles = useCallback((files: File[]) => {
    ...
    setRangeError(null);
    setRangeOpen(false);
    setOutputName(f.name.replace(/\.pdf$/i, '') + '-trimmed.pdf');
  }, []);
  ...
  const actionDisabled = (
    !file || !pdfLib || status === 'processing' || !pageCount ||
    selected.size === 0 || allSelected || isSinglePage
  );

  const helperText = (() => {
    if (isSinglePage) return 'Nothing to delete — this PDF has only one page.';
    if (allSelected) return 'Select at least one page to keep.';
    return null;
  })();
  ...
  const handleDelete = async () => {
    ...
    try {
      const blob = await deletePages(file, selected, pdfLib.PDFDocument);
      setOutputBlob(blob);
      setStatus('done');
    } catch (err: unknown) {
      if (err instanceof Error && err.message.toLowerCase().includes('encrypt')) {
        ...
        setErrorMsg(`Failed to delete pages. ${err instanceof Error ? err.message : 'Unknown error.'}`);
      }
      setStatus('error');
    }
  };
      ...
      {hasFile && status !== 'done' && (
        <>
          ...
          {pdfJsError && thumbs.length === 0 && (
            <p className="text-xs text-amber-600">Thumbnails unavailable. You can still delete by range below.</p>
          )}

          {/* Range input collapsible */}
          <div className="border border-slate-200 rounded-lg bg-white overflow-hidden">
            ...
            {rangeOpen && (
              <div id="range-panel" className="px-4 pb-4 space-y-2 border-t border-slate-100">
                <p className="text-xs text-slate-500 pt-3">
                  Enter page numbers to select for deletion (e.g. <code className="bg-slate-100 px-1 rounded">1-5, 8, 10-12</code>). Replaces current selection.
                </p>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-rose-400 focus:ring-2 focus:ring-rose-50 outline-none"
                    aria-label="Page range for selection"
                  />
                  <button
                    type="button"
                    onClick={handleRangeApply}
                    disabled={!rangeStr.trim() || !pageCount}
                    className="px-3 py-2 rounded-lg bg-rose-600 text-white text-sm font-medium hover:bg-rose-700 disabled:bg-slate-200 disabled:text-slate-400 transition-colors whitespace-nowrap"
                  >
                    Apply
                  </button>
                </div>
                {rangeError && (
                  <p className="text-xs text-rose-600 font-medium">{rangeError}</p>
                )}
              </div>
            )}
          </div>
          ...
          {helperText && (
            <p className="text-xs text-amber-700 font-medium px-1">{helperText}</p>
          )}
      ...
      {status === 'done' && outputBlob && (
        <div className="space-y-4">
          <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-emerald-50 border border-emerald-200 text-xs text-emerald-800">
            ...
            <span>Pages removed — {formatMB(outputBlob.size)} MB</span>
          </div>
