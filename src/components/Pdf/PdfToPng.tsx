...
async function convertPdfToPng(
  ...
  for (let i = 0; i < selectedIndices.length; i++) {
    ...
    const blob = await renderPageToPng(page, dpi, transparent);
    const url = URL.createObjectURL(blob);
    out.push({ name: `${base}-page-${String(pageNum).padStart(3, '0')}.png`, blob, url });
    page.cleanup();
...
// Estimated output size in bytes: PNG is lossless, ~3 bytes/px uncompressed, ~2x compression
function estimateOutputMb(
  ...
  // PNG compression ratio roughly 1/3 of raw
  const compressionRatio = 1 / 3;
...
export default function PdfToPng() {
  ...
  const estimatedMb =
    file && pageCount > 0 && selectedPages.size > 0
      ? estimateOutputMb(selectedPages.size, dpi)
      : 0;
          ...
          <div className="space-y-4">
            ...
            {showSizeWarning && (
              <div className="px-3 py-2.5 rounded-lg bg-amber-50 border border-amber-200 text-xs text-amber-800">
                Estimated output size is ~{estimatedMb.toFixed(0)} MB. PNG files are larger than JPG — at 300 DPI with many pages, this may be very large. Consider reducing resolution or selecting fewer pages.
              </div>
            )}
            ...
            >
              {status === 'processing'
                ...
                : `Convert ${selectedPages.size} page${selectedPages.size !== 1 ? 's' : ''} to PNG`}
            </button>
          </div>
          ...
          <div>
            <ToolCard title="Export options">
              <div className="space-y-4">
                {/* Resolution */}
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1.5" htmlFor="pdf-png-dpi">
                    Resolution
                  </label>
                  <select
                    id="pdf-png-dpi"
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-700 mb-1.5">Background</p>
                  <div className="flex rounded-lg border border-slate-200 overflow-hidden text-xs font-medium">
                    ...
                    <button
                      ...
                      className={`flex-1 px-3 py-2 border-l border-slate-200 transition-colors ${
                        transparent
                          ? 'bg-rose-600 text-white'
                          : 'bg-white text-slate-600 hover:bg-slate-50'
                      }`}
