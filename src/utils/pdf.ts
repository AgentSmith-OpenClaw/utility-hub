...
function addWatermark(pdf: jsPDF) {
  ...
  const disclaimerStr = 'For informational purposes only — verify results independently. Not professional financial advice.';
  pdf.setFontSize(6);
  pdf.setTextColor(160, 165, 175);
  pdf.text(disclaimerStr, MARGIN, pageH - 5, { maxWidth: pageW - 2 * MARGIN });
}
...
// ─── Disclaimer Box (last page) ────────────────────────────────────
function drawDisclaimerBox(pdf: jsPDF) {
  const pageW = pdf.internal.pageSize.getWidth();
  const pageH = pdf.internal.pageSize.getHeight();
  const boxH = 14;
  ...
  pdf.roundedRect(MARGIN, boxY, pageW - 2 * MARGIN, boxH, 1.5, 1.5, 'F');
  pdf.setFillColor(245, 158, 11);
  pdf.rect(MARGIN, boxY, 1.2, boxH, 'F');

  pdf.setFontSize(7);
  pdf.setFont('helvetica', 'bold');
  pdf.setTextColor(120, 53, 15);
  pdf.text('Disclaimer', MARGIN + 4, boxY + 4);

  pdf.setFont('helvetica', 'normal');
  pdf.setTextColor(146, 64, 14);
  const disclaimerText = 'This report is generated by toolisk.com for informational purposes only. Results are estimates based on user-provided inputs and standard formulas. Toolisk is not a financial advisor. Kindly review results before making financial decisions and consult a qualified professional for advice.';
  const lines = pdf.splitTextToSize(disclaimerText, pageW - 2 * MARGIN - 8);
  pdf.text(lines, MARGIN + 4, boxY + 7.5);
}
